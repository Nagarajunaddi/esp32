...
\title{ 
%	\logo{
	Resistance Measurement and Display using I2C
%	}
}
%\title{
...
%\maketitle
\begin{abstract}
Through this manual, we will learn how to setting up the I2C Communication between ESP32 and Arduino Uno.
\end{abstract}
\section{Components}
...
\item In this we are using ESP32 as a Master, and use two Arduino Uno boards as a slaves refered as Arduino$1$ and Arduino$2$.
...
%
\item
Connect the Arduino$2$ to LCD pins as per Table \ref{Table:1}.
\begin{table}[h]
\centering
\input{./figs/table2.tex}
\caption{}
\label{Table:1}
\end{table}
...
\item
Connect the ESP32 pins to Arduino$1$ and Arduino$2$ as per Table \ref{Table:2}.
\begin{table}[h]
\centering
\input{./figs/ESP32_table.tex}
\caption{}
\label{Table:2}
\end{table} 

\item Execute the following code to flash and program ESP32 as a Master.\\
\url {https://github.com/Nagarajunaddi/esp32/tree/main/I2C/1_Master%2C%202_Slaves/I2C_Master/src/main.cpp}

\item Execute the following code to flash and program Arduino$1$ as a Slave$1$.\\
\url {https://github.com/Nagarajunaddi/esp32/blob/main/I2C/1_Master%2C%202_Slaves/slave1/src/main.cpp}

\item Execute the following code to flash and program Arduino$2$ as a Slave$2$.\\
\url {https://github.com/Nagarajunaddi/esp32/blob/main/I2C/1_Master%2C%202_Slaves/slave2/src/main.cpp}
\end{enumerate}
...
\item
Connect the 5V pin of the Arduino$1$ to an extreme pin of the Breadboard shown in Fig. \ref{fig:breadboard}.  Let this pin be $V_{cc}$.
...
%
\item
Connect the GND pin of the Arduino$1$ to the opposite extreme pin of the Breadboard.
...
%
\item
Connect the junction between the two resistors to  the A0 pin on the Arduino$1$.
...
\item We create a variable called analog pin and assign it to 0. 
This is because the voltage value we are going to read is connected to analog pin A0.

\item  The 10-bit ADC can differentiate 1024 discrete voltage levels, 5 volt is applied to 2 resistors and the voltage sample is taken in between the resistors. The value which we get from analog pin can be between 0 and 1023. 0 would represent 0 volts falls across the unknown resistor. A value of 1023 would mean that practically all 5 volts falls across the unknown resistor.
...
\Rightarrow R_2&=\frac {R_1}{\brak{\frac{V_{in}}{V_{out}}-1}}
\end{align}
%
